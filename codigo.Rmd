...
$$
f(x) = \frac{x^{2-1} e^{-x/\theta}}{\theta^2 \Gamma(2)} = \frac{x e^{-x/\theta}}{\theta^2 \cdot 1} = \frac{x e^{-x/\theta}}{\theta^2} = \frac{x}{\theta^2} e^{-x/\theta}
$$
...
se saca el logaritmo natural a $L(\theta; x_1, x_2, \dots, x_n)$ utilizando las siguientes propiedades del logaritmo natural:
...
entonces se obtiene la expresion simplificada de la funcion de log-verosimilitud:

$$
\ell(\theta) = -2n \ln(\theta) + \sum_{i=1}^{n} \ln(x_i) - \frac{1}{\theta} \sum_{i=1}^{n} x_i.
$$

### Paso 3: se maxima la funcion de log verosimilitud respecto al parametro $\theta$.
...
Esto quiere decir que para la distribucion gamma cuando $k=2$ entonces el estimador de maxima verosimilitud es $\bar{x}/2$.

**b.**

Para enconrar el estimador usando el metodo de momentos para $\theta$ primero se igualan los momentos muestrales (media muestral) a los momentos poblacionales (la esperanza poblacional) para luego despejar $\theta$.

#### Paso 1: Calcular la esperanza poblacional $X$
...
Para este caso donde $k = 2$, entonces quedaria:
...
#### Paso 3: Se despeja $\theta$:

$$
\hat{\theta}_{MM} = \frac{\bar{X}}{2}
$$

Finalmente el estimador usando el metodo de momentos de $\theta$ es:
...
# configurar como se posicionan las graficas, de a dos columnas
par(mfrow = c(2,2))
